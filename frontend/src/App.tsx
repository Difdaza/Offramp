...
import {
  submitFlow, analyzeFlow, adjudicate, flagOrClear, fundPool,
  getCase, getCounts, getPoolBalance, listAll,
  FlowCaseView, FlowRow,
} from "./contractService";
import { CONTRACT_ADDRESS, VERIFIER_ADDRESS } from "./chain";
...
export function App() {
  ...
  const [targetUrl, setTargetUrl] = useState("https://example.com/account/cancel");
  const [htmlBundle, setHtmlBundle] = useState("");
  const [proofId, setProofId] = useState("");
  const [sessionProofHex, setSessionProofHex] = useState("");
  const [bond, setBond] = useState("1");
  const [poolTopup, setPoolTopup] = useState("1");
  ...
  async function onSubmit() {
    if (!acct || !walletClient) return;
    if (!targetUrl.trim().startsWith("https://")) return setNote("Target URL must be HTTPS.");
    if (htmlBundle.trim().length < 30) return setNote("Paste at least 30 chars of authenticated HTML evidence.");
    if (proofId.trim().length < 16) return setNote("Proof id must be at least 16 chars.");
    if (sessionProofHex.trim().replace(/^0x/i, "").length < 64) return setNote("Session proof must be at least 32 bytes as hex.");
    const id = await run("Submitting verified session evidence", () => submitFlow(walletClient, {
      targetUrl,
      htmlBundle,
      proofId,
      sessionProofHex,
      bondWei: BigInt(Math.max(1, Math.floor(Number(bond) || 1))),
    }));
    if (id != null) { setSelId(id); setNote(`Flow #${id} filed. Run analysis to count obstacles.`); }
  }
  async function onFundPool() {
    if (!walletClient) return;
    await run("Funding the bounty pool", () => fundPool(walletClient, BigInt(Math.max(1, Math.floor(Number(poolTopup) || 1)))));
  }
  async function onAnalyze() { if (!walletClient || selId == null) return; await run("Analyzing the flow", () => analyzeFlow(walletClient, selId)); }
  async function onAdjudicate() { if (!walletClient || selId == null) return; await run("Validators ruling the pattern", () => adjudicate(walletClient, selId)); }
  async function onFlagOrClear() { if (!walletClient || selId == null) return; await run("Settling the flow", () => flagOrClear(walletClient, selId)); }
    ...
    <div className="page">
      ...
      <section className="hero">
        <div className="hcopy">
          ...
          <p className="lede">
            Submit authenticated cancellation-session evidence. A panel of GenLayer validators classifies every{" "}
            <em>semantic dark pattern</em> between you and the unsubscribe button - hidden links,
            forced phone calls, retention loops. Four or more matched patterns flips the verdict to DARK_PATTERN.
          </p>
          <div className="meta">
            ...
            <span>verifier</span><button type="button" className="copybtn" aria-label="Copier l'adresse du verifier" onClick={() => copyText(VERIFIER_ADDRESS)}><code>{shortAddr(VERIFIER_ADDRESS)}</code> ⧉</button>
            <span className="sep">·</span>
            <span>verdicts</span><code>CLEAN · GREY · DARK_PATTERN</code>
          </div>
          <p className="prov">Source : HTML authentifié par verifier + session proof, puis jugé par consensus GenLayer via <code>gl.nondet</code>.</p>
        </div>
        <div className="hviz">
      ...
      <nav className="rule">
        <span><i>1</i> Submit verified session proof</span>
        <span><i>2</i> Analyze semantic patterns</span>
        <span><i>3</i> Validators rule the catalog</span>
      ...
      <section className="work">
        ...
        <aside className="side">
          <div className="panel">
            <h3>Submit verified evidence</h3>
            <label>Authenticated target URL</label>
            <input value={targetUrl} onChange={(e) => setTargetUrl(e.target.value)} placeholder="https://service.example/account/cancel" />
            <label>Authenticated HTML bundle</label>
            <textarea value={htmlBundle} onChange={(e) => setHtmlBundle(e.target.value)} placeholder={"<html>...cancel subscription page from a zkTLS/session proof...</html>"} />
            <label>Proof id</label>
            <input value={proofId} onChange={(e) => setProofId(e.target.value)} placeholder="session-proof-2026-..." />
            <label>Session proof bytes (hex)</label>
            <input value={sessionProofHex} onChange={(e) => setSessionProofHex(e.target.value)} placeholder="0x001122... at least 32 bytes" />
            <label>Bond (wei)</label>
            <input value={bond} onChange={(e) => setBond(e.target.value)} placeholder="1" />
            <button className="go" disabled={!isConnected || !walletClient || !!busy || !targetUrl.trim().startsWith("https://") || htmlBundle.trim().length < 30 || proofId.trim().length < 16 || sessionProofHex.trim().replace(/^0x/i, "").length < 64} onClick={onSubmit}>
              {isConnected ? "Post bond & submit verified proof" : "Connect a wallet to submit"}
            </button>
          </div>

          <div className="panel">
            <h3>Fund bounty pool</h3>
            <label>Top-up amount (wei)</label>
            <input value={poolTopup} onChange={(e) => setPoolTopup(e.target.value)} placeholder="1" />
            <button className="go" disabled={!isConnected || !walletClient || !!busy} onClick={onFundPool}>Fund pool</button>
          </div>
          ...
          {sel && selId != null && (
            <div className="panel selpanel">
              ...
              <div className="kv"><span>target</span><code>{sel.targetUrl}</code></div>
              <div className="kv"><span>proof</span><code>{sel.evidenceVerified ? "verified" : "not verified"} · {sel.proofId}</code></div>
              ...
              {sel.verdict ? (
                <>
                  ...
                  <div className="kv"><span>patterns</span><code>{sel.patternSignature || "-"}</code></div>
                  <div className="kv"><span>severity</span><code>{sel.maxSeverity} · {sel.regulatoryConcern || "NONE"}</code></div>
                  <div className="kv"><span>bounty paid</span><code>{sel.bountyPaid}</code></div>
                  {sel.rationale && <p className="rationale">{sel.rationale}</p>}
                  {sel.catalogReport && <pre className="rationale">{sel.catalogReport}</pre>}
                </>
              ...
              {sel.status === 0 && (<button className="go" disabled={!isConnected || !walletClient || !!busy} onClick={onAnalyze}>Analyze patterns</button>)}
              {sel.status === 1 && (<button className="go" disabled={!isConnected || !walletClient || !!busy} onClick={onAdjudicate}>Rule the pattern</button>)}
              {sel.status === 2 && (<button className="go" disabled={!isConnected || !walletClient || !!busy} onClick={onFlagOrClear}>Flag or clear · settle bond</button>)}
